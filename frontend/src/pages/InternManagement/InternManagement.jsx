...
export default function InternManagement() {
  ...
  const fetchInterns = async () => {
    setLoading(true);
    try {
      const data = await internsApi.adminList({
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      await internsApi.updateStatus(selectedIntern.intern_id, newStatus);
  ...
  const handleToggleReq = async (req) => {
    ...
    const actionText = newStatus === 'approved' ? 'mark as approved' : 'mark as pending';

    if (!confirm(`Are you sure you want to ${actionText} "${req.name}"?`)) return;

    try {
      await documentsApi.adminMark(selectedIntern.intern_id, req.document_type_id, newStatus);
      const data = await documentsApi.getInternRequirements(selectedIntern.intern_id);
      setRequirements(data.requirements || []);
      setReqStats(data.stats || reqStats);
    } catch (err) {
      alert(err.message);
    }
  };
  ...
  const filtered = interns.filter(i => {
    ...
    return (
      ...
      i.school?.toLowerCase().includes(term) ||
      i.department_name?.toLowerCase().includes(term)
    );
  });

  if (loading && !interns.length) {
    return <div className="loading-state"><div className="spinner" /><span>Loading interns…</span></div>;
  }

  return (
    <div className="manage-page" id="intern-management">
      <div className="manage-page__header">
        <div>
          <h2 className="manage-page__title">Intern Management</h2>
          <p className="manage-page__subtitle" style={{ fontSize: 'var(--text-label-md)', opacity: 0.7 }}>
            Overview of all registered interns and their OJT progress.
          </p>
        </div>
      </div>
      ...
      {selectedIntern && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '760px', width: '90vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                    <select
                      className="input"
                      style={{ fontSize: '11px', padding: '2px 8px', height: 'auto', width: 'auto' }}
                    ...
                    <span className={`chip chip--${selectedIntern.status === 'complete' ? 'success' : selectedIntern.status === 'terminated' ? 'error' : 'warning'}`} style={{ fontSize: '11px' }}>
                      Currently: {selectedIntern.status}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Tab Body */}
            <div className="modal__body" style={{ overflowY: 'auto', flex: 1, padding: '20px' }}>
              {/* --- DETAILS TAB --- */}
